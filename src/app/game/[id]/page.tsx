  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (num !== undefined) {
      if (num < 0 || num > highLimit) {
        ...
        // First, update the guess list
        const updatedGuessList = [...guessList, num];
        setGuessList(updatedGuessList);
        
        // Then update the number of guesses taken
        const updatedGuessesTaken = guessesTaken + 1;
        setGuessesTaken(updatedGuessesTaken);

        if (num === randomNumber) {
          // Player won
          setGameResult(
            `Congratulations! You guessed the number in ${
              updatedGuessList.length
            } attempts. Your score is ${Math.max(0, 100 - (updatedGuessList.length - 1) * 10)}`
          );
          setGameWon(true);
          setGamePlay(false);
        } else if (updatedGuessesTaken >= guessTotal) {
